...
@onready var los_area: Area2D = $LOSHulk
...
func chase_player (delta):
	if player.global_position.x > global_position.x:
		hulk.flip_h = true   # Player is to the right; face right (adjust as needed)
	else:
		hulk.flip_h = false    # Player is to the left; face left
